...
const AddressForm = ({ address, onCancel, setAddresses }) => {
    ...
    const onAddNewAddress = async (values) => {
        try {
            const newAddress = await addressService.create(values, authen?.user?.token)
    ...
    const onUpdateAddress = async (id, values) => {
        try {
            const updatedAddress = await addressService.update(id, values, authen?.user?.token)
            setAddresses(prev => prev.map(a => a.id !== id ? a : updatedAddress.data))
        } catch (err) {
            console.log(err)
        } finally {
            onCancel()
    ...
    return <div className={styles.formContainer}>
        <Formik
            ...
            onSubmit={(values, { setSubmitting }) => {
                setTimeout(() => {
                    if (address)
                        onUpdateAddress(address?.id, values)
                    else
