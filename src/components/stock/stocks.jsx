...
const Stocks = ({ product }) => {
    ...
    return <div className={styles.container}>
        <Button className={styles.addBtn} text="Add new stock" size="small" borderRadius="square" onClick={onAddStock} />
        <h2>Stock</h2>
        <table className={styles.table}>
            ...
            <tbody>
                {stocks.map(s => <tr className={styles.tableRow} key={s.id}>
                    <td>{product.title}</td>
                    <td>{s.size}</td>
                    <td><span className={styles.color} style={{ backgroundColor: s.color }}></span></td>
                    <td>{s.quantity}</td>
                    <td>
        ...
        {ReactDOM.createPortal(
            <Modal isOpen={openFormModal} size="medium" showButtonGroup={false}>
                <h5>{currentStock?.id ? "Update stock" : "Add new stock"}</h5>
                <p><strong>Product:</strong> {product.title} - <strong>ID: </strong>{product.id}</p>
                <StockForm stock={currentStock} productId={product?.id} onCancel={onCloseFormModal} setStocks={setStocks} />
            </Modal>,
