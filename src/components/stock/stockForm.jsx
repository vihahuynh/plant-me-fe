...
const StockForm = ({ stock, productId, onCancel, setStocks }) => {
    ...
    const onUpdateStock = async (values) => {
        try {
            ...
            const updatedStock = await stockSerice.update(stock?.id, stockToUpdate, authen?.user?.token)
    ...
    const onAddNewStock = async (values) => {
        try {
            ...
            const returnedStock = await stockSerice.create(newStock, authen?.user?.token)
                ...
                {({
                    ...
                    <form onSubmit={handleSubmit} className={styles.form}>
                        <div className={`${styles.inputContainer} ${styles.size}`}>
                            ...
                            <ul className={styles.sizeList}>
                                {["XS", "S", "M", "L", "XL"].map(item =>
                                    <li className={`${styles.sizeItem} ${currentSize === item ? styles.active : ""}`} key={item} onClick={() => setCurrentSize(item)}>{item}</li>
                                )}
                            </ul>
                        ...
                        <div className={styles.btnGroup}>
                            <Button text="Save" size="small" borderRadius="square" disabled={isSubmitting} />
                            <Button text="Cancel" size="small" borderRadius="square" theme="light" onClick={onCancel} />
                        </div>
                    </form>
                )}
            </Formik>
